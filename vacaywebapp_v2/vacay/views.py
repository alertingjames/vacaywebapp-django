...
def send_mail_tofriend(request):
    if request.method == 'POST':
        ...
        response = post_info(settings.SEVER_URL + '/makeMail', dt)
        result_code = response.get('result_code')
        if result_code == '0':
            ...
            try:
                ...
                files = {'file': photo}
                response2 = post_file(settings.SEVER_URL + '/uploadMailPhoto', files, param)

                result_code = response2.get('result_code')
                if result_code == '0':
                    response = post_info(settings.SEVER_URL + '/sendMailMessage', param)
    ...
    if request.method == 'POST':
        ...
        if result_code == '0':
            ...
            try:
                ...
                response = post_info(settings.SEVER_URL + '/sendMailMes', param2)
                result = response.get('result')
                context = {
                    'msg': msg,
                    'me_name': me_name,
