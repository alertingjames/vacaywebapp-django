...
from vacay.models import Employee, InfoBuffer, CommonUser, MailBox, EatDrink, Job, Announce, Media, Watercooler, \
    Comment, Service, ProviderSchedule, Product, Contactor, Provider, AdminUser, RetailProduct, RetailDetail, UserPhoto
...
def welcome(request):
    ...
    try:
        if request.session['em_email'] != '':
            email = request.session['em_email']
            employees = Employee.objects.filter(email=email)
            if employees.count() > 0:
                request.session['em_company'] = employees[0].company
                return render(request, 'vacay/home.html', {'me_email': email})
            else:
                return redirect('/logout')
        elif request.session['pro_email'] != '':
            email = request.session['pro_email']
            providers = Provider.objects.filter(proEmail=email)
            if providers.count() > 0:
                return render(request, 'vacay/provider_home.html', {'me': providers[0]})
            else:
                return redirect('/logout')
        ...
        try:
            ...
            if s1 != '' or s2 != '' or s3 != '' or s4 != '' or s5 != '':
                ...
                if request.session['em_email'] != '':
                    ...
                    employees = Employee.objects.filter(email=eml)
                    employee = employees[0]
                    nameStr = employee.name.split()
                    ...
                    user.photo_url = employee.image
                    user.em_millennial = employee.millennial
                elif request.session['fb_email'] != '':
                ...
                commonusers = CommonUser.objects.filter(email=eml)
                ...
                return render(request, 'vacay/login_survey.html')
...
@permission_classes((AllowAny,))
@api_view(['GET', 'POST'])
def employee_post_profile(request):
    eml = ''
    picture = ''
    name = ''
    email = ''
    ...
    if request.method == 'POST':
        ...
        if request.session['em_email'] != '':
            ...
            millennial = request.POST.get('millennial', '')
        elif request.session['fb_email'] != '':
        ...
        golf = request.POST.get('golf', None)
        tennis = request.POST.get('tennis', None)
        ski = request.POST.get('ski', None)
        biking = request.POST.get('biking', None)
        fishing = request.POST.get('fishing', None)
        surfing = request.POST.get('surfing', None)
        exploring = request.POST.get('exploring', None)

        rwalk = request.POST.get('walk', None)
        r30mins = request.POST.get('30mins', None)
        r1mile = request.POST.get('1mile', None)
        r5miles = request.POST.get('5miles', None)
        r10miles = request.POST.get('10miles', None)

        gbeginner = request.POST.get('beginner', None)
        gintermediate = request.POST.get('intermediate', None)
        gadvanced = request.POST.get('advanced', None)
        gscramble = request.POST.get('scramble', None)

        t3p = request.POST.get('3p', None)
        t4p = request.POST.get('4p', None)
        t5p = request.POST.get('5p', None)
        t6p = request.POST.get('6p', None)
        twtf = request.POST.get('wtf', None)

        shot = request.POST.get('hot', None)
        sbunny = request.POST.get('bunny', None)
        sgreens = request.POST.get('greens', None)
        sblues = request.POST.get('blues', None)
        sblacks = request.POST.get('blacks', None)
        sdouble = request.POST.get('double', None)
        snotafraid = request.POST.get('notafraid', None)

        bshort = request.POST.get('short', None)
        blong = request.POST.get('long', None)
        bmountain = request.POST.get('mountain', None)
        broad = request.POST.get('road', None)

        ffly = request.POST.get('fly', None)
        flake = request.POST.get('lake', None)
        focean = request.POST.get('ocean', None)

        surocean = request.POST.get('oceansurfing', None)
        surlake = request.POST.get('lakesurfing', None)
        kitesurfing = request.POST.get('kitesurfing', None)

        emuseums = request.POST.get('museums', None)
        ecitytours = request.POST.get('citytours', None)
        enature = request.POST.get('nature', None)
        eart = request.POST.get('art', None)
        econcerts = request.POST.get('concerts', None)
        electures = request.POST.get('lectures', None)
        ...
        try:
            ...
            picture = settings.URL + uploaded_file_url
        ...
        if bufs.count() > 0:
            ...
            if users.count() == 0:
                ...
                contactors = Contactor.objects.filter(email=email)
            ...
            response = post_info(settings.SERVER_URL + '/registerUser', dt)
...
def home(request):
    if request.session['em_email'] != '':
        em_email = request.session['em_email']
        employees = Employee.objects.filter(email=em_email)
        if employees.count() > 0:
            request.session['em_company'] = employees[0].company
            return render(request, 'vacay/home.html', {'me_email': em_email})
        else:
            return redirect('/logout')
    elif request.session['pro_email'] != '':
        email = request.session['pro_email']
...
def chat_page(request):
    if request.method == 'POST':
        ...
        if result_code == '0':
            ...
            contactors = []
            if request.session['em_email'] != '':
                contactors = Contactor.objects.filter(email=friend_email, user_email=request.session['em_email'])
            elif request.session['fb_email'] != '':
                contactors = Contactor.objects.filter(email=friend_email, user_email=request.session['fb_email'])
            elif request.session['pro_email'] != '':
                contactors = Contactor.objects.filter(email=friend_email, user_email=request.session['pro_email'])
            elif request.session['manager_email'] != '':
                contactors = Contactor.objects.filter(email=friend_email, user_email=request.session['manager_email'])
            if contactors.count() == 0:
                contactor = Contactor()
                if request.session['em_email'] != '':
                    contactor.user_email = request.session['em_email']
                elif request.session['fb_email'] != '':
                    contactor.user_email = request.session['fb_email']
                elif request.session['pro_email'] != '':
                    contactor.user_email = request.session['pro_email']
                elif request.session['manager_email'] != '':
                    contactor.user_email = request.session['manager_email']
                contactor.name = friend_name
                contactor.email = friend_email
                contactor.photo = friend_photo
                contactor.save()
            eml = ''
            if request.session['em_email'] !='':
                eml = request.session['em_email']
                ...
                eml = request.session['pro_email']
                users = Provider.objects.filter(proEmail=eml)
                user = users[0]
                me = CommonUser()
                me.userid = user.proid
                me.first_name = user.proFirstName
                me.last_name = user.proLastName
                me.email = user.proEmail
                me.photo_url = user.proProfileImageUrl
                return render(request, 'vacay/chat.html', {'me': me, 'friend': friend})
            elif request.session['manager_email'] !='':
                eml = request.session['manager_email']
                ...
                me.first_name = first_name
                me.last_name = last_name
                me.email = user.adminEmail
                me.photo_url = user.adminImageUrl
                return render(request, 'vacay/chat.html', {'me': me, 'friend': friend})
            else:
                return redirect('/home')
        else:
...
def chat_contactor(request, contactor_id):
    contactor = Contactor.objects.get(id=contactor_id)
...
def get_all_users(request):
    ...
    if result_code == '0':
        ...
        for user in userInfo:
            ...
            friend.relationship = '-' + user['relationship'].replace('\ncommon', '').replace('\n', '\n-')
    ...
    if result_code == '0':
        ...
        for user in userInfo:
            if user['userid'] == friend_id:
                ...
                friend.user_lat = user['user_lat']
                friend.user_lon = user['user_lon']
                return render(request, 'vacay/user_profile.html', {'friend': friend, 'mail':'true'})
    ...
    if result_code == '0':
        ...
        for user in userInfo:
            if user['userid'] == friend_id:
                ...
                friend.user_lat = user['user_lat']
                friend.user_lon = user['user_lon']
                return render(request, 'vacay/user_profile.html', {'friend': friend, 'chat':'true'})
    ...
    if request.method == 'POST':
        ...
        if mode == 'mail':
            ...
            if result_code == '0':
                ...
                me.email = userInfo[0]['email']
                me.photo_url = userInfo[0]['photo_url']
                me.age = userInfo[0]['age']
                me.job = userInfo[0]['job']
                me.address = userInfo[0]['address']
                me.relationship = userInfo[0]['relationship']
                me.education = userInfo[0]['education']
                me.interests = "-" + userInfo[0]['interests'].replace("{", "").replace("}", "").replace("\",",
                                                                                                        "\n-").replace(
                    "\"", "").replace("[", "").replace("]", "")
                me.survey = userInfo[0]['survey']
                request.session['friend_email'] = friend.email
                request.session['friend_photo'] = friend.photo_url
...
def mail_inbox(request):
    ...
    if request.session['em_email'] != '':
        ...
        eml = request.session['fb_email']
    mails = get_info(settings.SERVER_URL + "/getMail", {'email': eml})
    result_code = mails['result_code']
    # return HttpResponse(result_code)
    if result_code == '0':
        mailInfo = mails['mail_infos']
        for mail in mailInfo:
            ...
            mailList.insert(0, inbox)

        return render(request, 'vacay/mailbox.html', {'mails':mailList})
    elif result_code == '109':
        return render(request, 'vacay/mailbox.html', {'mails': []})
    else:
        return redirect('/get_all_users')
            ...
            if result_code == '0':
                ...
                for mail in mailInfo:
                    if mail['mail_id'] == mail_id:
                        ...
                        inbox.lat_message = mail['lat_message']
                        inbox.lon_message = mail['lon_message']
                        return render(request, 'vacay/mail_detail.html', {'mail': inbox, 'notloc':'true'})
            else:
                return redirect('/mail_inbox')
    ...
    if result_code == '0':
        ...
        for mail in mailInfo:
            if mail['mail_id'] == mail_id:
                ...
                friend_email = mail['from_mail']
                friend_photo = mail['photo_url']
...
def sent_mail(request):
    ...
    if request.session['em_email'] != '':
        ...
        eml = request.session['fb_email']
    mails = get_info(settings.SERVER_URL + "/allSentMail", {'email': eml})
    result_code = mails['result_code']
    # return HttpResponse(result_code)
    if result_code == '0':
        mailInfo = mails['sent_mail_infos']
        for mail in mailInfo:
            inbox = MailBox()
            inbox.mail_id = mail['mail_id']
        ...
        return render(request, 'vacay/sent_mail.html', {'mails':mailList})
    elif result_code == '110':
        return render(request, 'vacay/sent_mail.html', {'mails': []})
    else:
        return redirect('/get_all_users')
    ...
    if result_code == '0':
        ...
        for mail in mailInfo:
            if mail_id == mail['mail_id']:
                ...
                return render(request, 'vacay/sent_mail_detail.html', {'mail':inbox})
    else:
            ...
            if result_code == '0':
                ...
                for mail in mailInfo:
                    if mail_id == mail['mail_id']:
                        ...
                        return render(request, 'vacay/sent_mail_detail.html', {'mail': inbox, 'notloc':'true'})
            else:
                return redirect('/sent_mail')
...
def send_mail_tofriend(request):
    if request.method == 'POST':
        ...
        response = post_info(settings.SERVER_URL + '/makeMail', dt)
        result_code = response.get('result_code')
        if result_code == '0':
            ...
            try:
                ...
                files = {'file': photo}
                response2 = post_file(settings.SERVER_URL + '/uploadMailPhoto', files, param)

                result_code = response2.get('result_code')
                if result_code == '0':
                    response = post_info(settings.SERVER_URL + '/sendMailMessage', param)
                    result = response.get('result')
                    if result == '0':
                        if request.session['em_email'] != '':
                            param2 = {'em_id': em_id}
                            response = post_info(settings.SERVER_URL+'/addEmInteraction', param2)
...
def myprofile(request):
    ...
    if result_code == '0':
        ...
        me.photo_url = userInfo[0]['photo_url']
        me.age = userInfo[0]['age']
        me.phone_number = userInfo[0]['phone_number']
        me.job = userInfo[0]['job']
        me.address = userInfo[0]['address']
        me.relationship = '-' + userInfo[0]['relationship'].replace('\ncommon', 'common').replace('\n', '\n-')
        me.education = userInfo[0]['education']
        me.interests = "-" + userInfo[0]['interests'].replace("{", "").replace("}", "").replace("\",",
                                                                                                "\n-").replace(
            "\"", "").replace("[", "").replace("]", "")
    ...
    if result == '0':
        ...
        for jobinfo in jobInfo:
            if job_id == jobinfo['job_id']:
                ...
                if job.survey.startswith("http") and "?usp=" in job.survey:
                    index = job.survey.find("?usp=")
                    s = job.survey
                    job.survey = s[0:index] + "?embedded=true"
    ...
    if result == '0':
        ...
        for announceinfo in announceInfo:
            if announceinfo['an_id'] == announce_id:
                ...
                if announce.survey.startswith("http") and "?usp=" in announce.survey:
                    index = announce.survey.find("?usp=")
                    s = announce.survey
                    announce.survey = s[0:index] + "?embedded=true"
...
def get_watercoolerInfo(category, company, myemail):
    data = {
        'category': category,
        'company': company
    }
    ...
    if result == '0':
        ...
        for watercoolerinfo in watercoolerInfo:
            ...
            wc.email = watercoolerinfo['email']
...
def get_wc(request):
    ...
    if request.session['fb_email'] != '':
        ...
        email = request.session['em_email']
    data = get_watercoolerInfo(category, em_company, email)
    if data is not None:
...
def watercooler_setup(request):
    ...
    if request.session['em_email'] != '':
        employee = 'true'
    return render(request, 'vacay/watercooler_setup.html', {'employee':employee})


def editwc(request, wc_id):
    request.session['wc_id'] = wc_id
    category = request.session['wc_category']
    if request.session['em_email'] != '':
        company = request.session['em_company']
        param = {
            'category': category,
            'company': company
        }
    ...
    response = get_info(settings.SERVER_URL + "/get_watercooler", param)
    result = response.get('result_code')
    if result == '0':
        watercoolerInfo = response.get('data')
        for watercoolerinfo in watercoolerInfo:
            if watercoolerinfo['id'] == int(wc_id):
                wc = Watercooler()
                wc.wc_id = watercoolerinfo['id']
                wc.name = watercoolerinfo['name']
                wc.email = watercoolerinfo['email']
                ...
                wc.category = watercoolerinfo['category']
                wc.company = watercoolerinfo['company']
                wc.link = watercoolerinfo['link']
                employee = ''
                if request.session['em_email'] != '':
                    employee = 'true'
                return render(request, 'vacay/edit_watercooler.html', {'wc':wc, 'employee':employee})
    else:
        return redirect('/home', {'error':'true'})
...
@permission_classes((AllowAny,))
@api_view(['GET', 'POST'])
def updatewatercooler(request, wc_id):
    if request.method == 'POST':
        ...
        response = post_info(settings.SERVER_URL + '/updatewatercooler', data)
        result = response.get('result_code')
        if result == '0':
            # return redirect('/home')
            return redirect('/get_wc?category=' + category)
        else:
            return HttpResponse('Server issue!')
...
def delwc(request, wc_id):
    ...
    response = post_info(settings.SERVER_URL + '/delwatercooler', data)
    result = response.get('result_code')
    if result == '0':
        # return redirect('/home')
        return redirect('/get_wc?category=' + request.session['wc_category'])
    else:
        return HttpResponse('Server issue!')
...
def delcomment(request, comment_id):
    ...
    response = post_info(settings.SERVER_URL + '/delcomment', data)
    result = response.get('result_code')
    if result == '0':
        # return redirect('/home')
        return redirect('/get_wc?category=' + request.session['wc_category'])
...
def upload_watercooler(request):
    if request.method == 'POST':
        ...
        content = request.POST.get('description', None)
        ...
        if request.session['em_email'] != '':
            ...
            if category == '' or category is None:
                ...
                response = post_info(settings.SERVER_URL + '/upload_watercooler', data)
                result = response.get('result_code')
                if result == '0':
                    # context = {'item': 'uploadwatercooler',
                    #           'response': 'Success!'}
...
def comments(request, wc_id):
    ...
    data = {'info_id': wc_id}
    param = {}
    response = get_info(settings.SERVER_URL + '/get_comment', data)
    result = response.get('result_code')
    if result == '0':
        commentInfo = response.get('data')
        for commentinfo in commentInfo:
            comment = Comment()
            comment.comment_id = commentinfo['id']
            comment.info_id = commentinfo['info_id']
            comment.photoUrl = commentinfo['photoUrl']
            comment.name = commentinfo['name']
            comment.email = commentinfo['email']
            comment.text = commentinfo['text']
            comment.imageUrl = commentinfo['imageUrl']
            if request.session['em_email'] != '':
                if comment.email == request.session['em_email']:
                    comment.whose = 'me'
            elif request.session['fb_email'] != '':
                if comment.email == request.session['fb_email']:
                    comment.whose = 'me'
        ...
        if result == '0':
            ...
            for watercoolerinfo in watercoolerInfo:
                if watercoolerinfo['id'] == int(wc_id):
                    ...
                    wc.link = watercoolerinfo['link']
                    if request.session['em_email'] != '':
                        if wc.email == request.session['em_email']:
                            wc.whose = 'me'
                        return render(request, 'vacay/comments.html', {'comments': commentList, 'watercooler': wc})
                    elif request.session['fb_email'] != '':
                        if wc.email == request.session['fb_email']:
                            wc.whose = 'me'
                        return render(request, 'vacay/incomments.html', {'comments': commentList, 'info': wc})
        else:
            return redirect('/home', {'error':'true'})
    else:
        return HttpResponse('<h1>Server failed</h1>')
    ...
    if result == '0':
        ...
        for watercoolerinfo in watercoolerInfo:
            if watercoolerinfo['id'] == int(wc_id):
                ...
                return render(request, 'vacay/add_comment.html', {'watercooler': wc, 'employee':employee})
    else:
        return redirect('/home')


def editcomment(request, wc_id, comment_id):
    category = request.session['wc_category']
    company = 'common'
    employee = ''
    if request.session['em_email'] != '':
    ...
    if result == '0':
        commentInfo = response.get('data')
        for commentinfo in commentInfo:
            if commentinfo['id'] == int(comment_id):
                comment = Comment()
                comment.comment_id = commentinfo['id']
                comment.info_id = commentinfo['info_id']
                comment.photoUrl = commentinfo['photoUrl']
                ...
                if result == '0':
                    ...
                    for watercoolerinfo in watercoolerInfo:
                        if watercoolerinfo['id'] == int(wc_id):
                            ...
                            return render(request, 'vacay/add_comment.html', {'watercooler': wc, 'commentObj':comment, 'employee':employee})
                    return redirect('/home')
                else:
                    return redirect('/home')
        return redirect('/home')
...
def returnfromdrawing(request):
    if request.method == 'POST':
        ...
        if result == '0':
            watercoolerInfo = response.get('data')
            for watercoolerinfo in watercoolerInfo:
                if watercoolerinfo['id'] == int(request.session['wc_id']):
                    wc = Watercooler()
                    wc.wc_id = watercoolerinfo['id']
                    wc.name = watercoolerinfo['name']
                    wc.email = watercoolerinfo['email']
                    ...
                    return render(request, 'vacay/add_comment.html',
                                  {'watercooler': wc, 'image': imageURL, 'comment': text, 'employee':employee})
        else:
            return redirect('/home', {'error':'true'})
    elif request.method == 'GET':
        pass
...
def wc_comment_upload(request):
    if request.method == 'POST':
        ...
        response = post_info(settings.SERVER_URL + '/upload_comment', data)
        result = response.get('result_code')
        if result == '0':
    ...
    if request.method == 'POST':
        ...
        if result_code == '0':
            ...
            try:
                ...
                response = post_info(settings.SERVER_URL + '/sendMailMes', param2)
                result = response.get('result')
                context = {
                    'msg': msg,
                    'me_name': me_name,
...
import stripe         #########################################################################################################       pip install --index-url https://code.stripe.com --upgrade stripe
...
def inbox(request):
    ...
    if result_code == '0':
        ...
        return render(request, 'vacay/inbox.html', {'mails': mailList})
    elif result_code == '109':
        return render(request, 'vacay/inbox.html', {'mails': []})
    else:
        return redirect('/home')
...
def indetail(request, mail_id):
    ...
    if request.session['em_email'] != '':
        ...
        users = AdminUser.objects.filter(adminEmail=eml)
        me = users[0]
        me_name = me.adminName
        me_email = me.adminEmail
        me_photo = me.adminImageUrl
    mails = get_info(settings.SERVER_URL + "/getMail", {'email': eml})
    result_code = mails['result_code']
    # return HttpResponse(result_code)
    if result_code == '0':
...
def sentbox(request):
    ...
    if result_code == '0':
        ...
        return render(request, 'vacay/sentbox.html', {'mails':mailList})
    elif result_code == '110':
        return render(request, 'vacay/sentbox.html', {'mails': []})
    else:
        return redirect('/home')
    ...
    if result_code == '0':
        ...
        for mail in mailInfo:
            if mail_id == mail['mail_id']:
                ...
                return render(request, 'vacay/sentdetail.html', {'mail':inbox})
    else:
        return redirect('/sent_mail')
...
def intofriend(request):
    if request.method == 'POST':
        ...
        if request.session['em_email'] != '':
            ...
            me_email = request.session['pro_email']
            users = Provider.objects.filter(proEmail=me_email)
            me = users[0]
            me_name = me.proFirstName + ' ' + me.proLastName
            me_photo = me.proProfileImageUrl
            me_id = me.proid
        elif request.session['manager_email'] != '':
            me_email = request.session['manager_email']
            users = AdminUser.objects.filter(adminEmail=me_email)
            me = users[0]
            me_name = me.adminName
            me_photo = me.adminImageUrl
            me_id = me.adminID
    ...
    if result_code == '0':
        ...
        for user in userInfo:
            if user['userid'] == user_id:
                ...
                return render(request, 'vacay/incompose.html', {'friend_email':request.session['friend_email'], 'friend_photo': request.session['friend_photo']})
    else:
        return redirect('/home')
...
def register_manager(request):
    if request.method == 'POST':
        ...
        if type == 'company':
            ...
            if result == '0':
                ...
                try:
                    ...
                    try:
                        ...
                        h = request.POST.get('h', '32')
                        #  return HttpResponse(w)
                        file = profile_process(photo, x, y, w, h)
                        filename = fs.save(file.name, file)
                        uploaded_file_url = fs.url(filename)

                        pic_url = settings.URL + uploaded_file_url
                    ...
                    if pic_url == '':
                        ...
                        response = post_info(settings.SERVER_URL + '/postAdminPhoto', param)
                        result = response.get('result_code')
            ...
            print(name + "/" + email + "/" + password + "/" + retail)
    ...
    if request.method == 'POST':
        try:
            ...
            if tosurvey == 'true':
                ...
                context = {'registered': 'false'}
                users = get_info(settings.SERVER_URL + "/getUserProfile", {'email': request.session['fb_email']})
                result_code = users['result_code']
                if result_code == '0':
                    userInfo = users['user_profile']
                    if request.session['fb_email'] == userInfo[0]['email']:
                        context = {'registered': 'true'}
                return render(request, 'vacay/login_survey.html', context)
            else:
...
def getinfo(request):
    ...
    if request.session['fb_email'] != '':
        ...
        email = request.session['em_email']
    data = get_watercoolerInfo(category, 'common', email)
    if data is not None:
        return render(request, 'vacay/info_list.html', {'infos':data, 'category':category})
    else:
        return redirect('/home')
...
def editprofilepicture(request):
    if request.method == 'POST':
        ...
        if users.count() > 0:
            user = users[0]
            user.photo_url = settings.URL + uploaded_file_url
            user.save()

        contactors = Contactor.objects.filter(email=email)
        if contactors.count() > 0:
            c = contactors[0]
            c.photo = settings.URL + uploaded_file_url
            c.save()

        employees = Employee.objects.filter(email=email)
        if employees.count() > 0:
            em = employees[0]
            em.image = settings.URL + uploaded_file_url
            em.save()

        data = {
            'email': email,
            'photo_url': settings.URL + uploaded_file_url
        }
        response = post_info(settings.SERVER_URL + '/editUserPhoto', data)
        result = response.get('result_code')
        return redirect('/home')
...
@permission_classes((AllowAny,))
@api_view(['GET', 'POST'])
def providereditprofilepicture(request):
    if request.method == 'POST':
        email = request.POST.get('email', '')
        photo = request.FILES['photo']
        fs = FileSystemStorage()
        ...
        response = post_info(settings.SERVER_URL + '/editProviderProfilePhoto', data)
        result = response.get('result_code')
        return redirect('/home')
...
def show_weather(request):
    ...
    return render(request, 'vacay/geocodetest.html')
...
def testmap(request):
    return render(request, 'vacay/on_map.html')
